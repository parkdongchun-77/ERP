...
## Phase 8 — 인사/급여
- [x] 사원 기본급 — 사원 마스터에 필드 추가. 근태/연차는 미구현
- [x] payroll_rates + 급여대장 생성 — SQL 검증: 3,000,000 → 공제 381,123 → 실지급 2,618,877. 요율은 화면에서 수정 가능
- [x] 급여대장 상세(명세 요약) — 화면 구현. 사원별 개별 명세서 PDF는 미구현(브라우저 인쇄로 대체 가능)
- [x] 급여 확정 자동분개 — SQL 검증: 분개 1건, 시산표 균형 유지

## Phase 9 — 그룹웨어 + 마무리
- [x] 전자결재 — 단일 결재자 간이 구현(요청/승인/반려). 다단계 결재선·전표 확정 연동은 미구현
- [x] 게시판 — 작성/삭제 구현. 일정(캘린더)은 미구현
