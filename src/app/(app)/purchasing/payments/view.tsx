// 지급 등록 폼 + 미지급금 현황 + 최근 지급 이력 클라이언트 뷰
"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { savePayment } from "@/app/(app)/sales/actions";
...
export function PaymentsView({ balances, recent }: { balances: Balance[]; recent: Recent[] }) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    const { error } = await savePayment({ ...form, amount: Number(form.amount) });
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-lg font-bold">지급/미지급</h1>
      ...
      <form onSubmit={submit} className="flex flex-wrap items-end gap-2 rounded border bg-white p-4">
        ...
        <div>
          <label className="mb-1 block text-xs text-gray-500">지급일</label>
          <input
            type="date"
        ...
        <div>
          <label className="mb-1 block text-xs text-gray-500">지급액</label>
          <input
            required
            type="number"
            min="1"
      ...
      <section>
        <h2 className="mb-2 text-sm font-semibold text-gray-700">거래처별 미지급금 현황</h2>
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b bg-gray-50 text-left">
              <th className="p-2">거래처</th>
              <th className="p-2 text-right">매입 합계</th>
              <th className="p-2 text-right">지급 합계</th>
              <th className="p-2 text-right">미지급 잔액</th>
            </tr>
          </thead>
          <tbody>
            {balances.length === 0 ? (
              ...
              balances.map((b) => (
                <tr key={b.partner_id} className="border-b">
                  ...
                  <td className="p-2 text-right">{b.purchase_total.toLocaleString()}</td>
                  <td className="p-2 text-right">{b.paid_total.toLocaleString()}</td>
                  <td className={`p-2 text-right font-medium ${b.balance > 0 ? "text-red-600" : ""}`}>
      ...
      <section>
        <h2 className="mb-2 text-sm font-semibold text-gray-700">최근 지급 이력</h2>
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b bg-gray-50 text-left">
              <th className="p-2">지급일</th>
              <th className="p-2">거래처</th>
