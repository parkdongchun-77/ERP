// 판매현황 보고서 (기간 내 확정 판매의 거래처별·품목별 집계)
...
export default async function SalesReportPage({
  ...
  const { data: sales, error } = await supabase
    .from("sales")
    .select(
      "doc_date, partners(name), sales_lines(qty, supply_amount, vat_amount, items(item_code, name))"
    )
    .eq("status", "confirmed")
    .gte("doc_date", from)
    .lte("doc_date", to);
  ...
  for (const s of sales ?? []) {
    const partnerName = (s.partners as unknown as { name: string } | null)?.name ?? "?";
    for (const l of (s.sales_lines as unknown as Line[]) ?? []) {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-lg font-bold">판매현황</h1>
      <form className="flex items-end gap-2">
        ...
        <span className="ml-4 text-sm text-gray-600">
          기간 매출 합계(부가세 포함): <b>{grandTotal.toLocaleString()}원</b>
        </span>
      </form>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section>
          <h2 className="mb-2 text-sm font-semibold text-gray-700">거래처별 매출</h2>
        ...
        <section>
          <h2 className="mb-2 text-sm font-semibold text-gray-700">품목별 매출</h2>
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b bg-gray-50 text-left">
